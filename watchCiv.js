...
const { compile, parse } = require("@danielx/civet"); // Compile civet code
...
// `
// ^ See footnote [1]
const OUTPUT_EXTENSION = ".ts"; // See footnote [2]

async function compileFile(path) {
  const outputPath = path
    .replace(/(\.civet)$/, OUTPUT_EXTENSION)
    .replace(`src/`, `servers/home/`);
